...
module Russian
  ...
  DE_LOWER_SINGLE = LOWER_SINGLE.invert

  LOWER_MULTI = {
    "ье"=>"ie",
    "ьё"=>"ie",
  }
  DE_LOWER_MULTI = LOWER_MULTI.invert
  ...
  DE_UPPER_SINGLE = UPPER_SINGLE.invert

  UPPER_MULTI = {
    "ЬЕ"=>"IE",
    "ЬЁ"=>"IE",
  }
  DE_UPPER_MULTI = UPPER_MULTI.invert

  LOWER = (LOWER_SINGLE.merge(LOWER_MULTI)).freeze
  DE_LOWER = LOWER.invert

  UPPER = (UPPER_SINGLE.merge(UPPER_MULTI)).freeze
  DE_UPPER = UPPER.invert

  MULTI_KEYS = (LOWER_MULTI.merge(UPPER_MULTI)).keys.sort_by {|s| s.length}.reverse.freeze
  DE_MULTI_KEYS = (DE_LOWER_MULTI.merge(DE_UPPER_MULTI)).keys.sort_by {|s| s.length}.reverse.freeze
  ...
  def detransliterate(str)
    chars = str.scan(%r{#{DE_MULTI_KEYS.join '|'}|\w|.})

    result = ""

    chars.each_with_index do |char, index|
      if DE_UPPER.has_key?(char) && DE_LOWER.has_key?(chars[index+1])
        # combined case
        result << DE_UPPER[char].downcase.capitalize
      elsif DE_UPPER.has_key?(char)
        result << DE_UPPER[char]
      elsif DE_LOWER.has_key?(char)
        result << DE_LOWER[char]
      else
        result << char
      end
    end
